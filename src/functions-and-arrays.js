...
function averageNumbers(numbersAvg) {
  let suma = 0;
  suma = sumArray(numbersAvg);
  let long = numbersAvg.length;
  let average;
  if (long != 0) {
    return (average = suma / long);
  }
}
...
function averageWordLength(wordsArr) {
  let numeroLetrasPalabra = wordsArr.map(function(element) {
    return element.length;
  });
  let totalPalabras = wordsArr.length;
  if (totalPalabras != 0) {
    return sumArray(numeroLetrasPalabra) / totalPalabras;
  }
}
...
function doesWordExist(wordsFind, wordToFind) {
  ...
  if (wordsFind.length != 0) {
    exists = false;
    while (exists === false && i <= wordsFind.length) {
...
function selectArrayVertical(matrix, i, j) {
  let array = [];
...
function greatestProduct(matrix) {
  ...
  for (let i = 0; i < 20; i++) {
    for (let j = 0; j < 20; j++) {
      ...
      if (i < 17 && j < 20) {
        selectedArrayV = selectArrayVertical(matrix, i, j);
        maxProduct = compareMaxProduct(selectedArrayV, product, maxProduct);
      }
    }
  }
  return maxProduct;
